...
contract maskSaver is ERC721 {
    ...
    function maskMaking(address _manuAddress) public returns (bool) {
        require(owner == msg.sender);
        
        Masks.push(Mask(now));
        uint256 tokenId = Masks.length - 1; 
        _mint(_manuAddress, tokenId);
        emit Makemask(tokenId, owner, "Making masks...");
        
        return true;
    }
    
    function dealMasks(address _to, uint256 _tokenId) public returns (bool) {
    ...
    function sellMasks(uint256 _tokenId) public {
        require(ownerOf(_tokenId) == msg.sender);
    ...
    function tokenByList(address _address) public returns (uint256[] memory) {
        require(owner == msg.sender);
        
        uint256 _index = balanceOf(_address);
        uint256[] memory _tokenIDList;
        
        _list = _tokenIDList;
        
        for(uint256 i = 0; i < _index; i++) {
            _list.push(tokenOfOwnerByIndex(_address, i));
        }
