...
contract MaskSaver {
    ...
    function startMaskMaking(uint256 _MaskQuantity) external onlyOwner {
        ...
        while (temp >= 500) {
            ...
            // 2. masks[index].serialNumber = generated serialNumber
            masks[index].manufacturerAddr = msg.sender;
            masks[index].productionDate = now;
            temp = temp.sub(500);
            index++;    
        }
    ...
    function stopMaskMaking() external onlyOwner {
        factoryOn = false;
        manufacturers[msg.sender].maskAmount = manufacturers[msg.sender].maskAmount.add(newMaskAmount);
        newMaskAmount = 0;
        emit MaskProduction(manufacturers[msg.sender].manufacturerName, manufacturers[msg.sender].maskAmount, "Finished making masks!");
    }
    ...
    function getMaskProductionDate(uint256 _index) external view returns (uint256) {
        return masks[_index].productionDate;
    }
    ...
    function getMaskQuantity(address _manufacturerAddr) external view returns (uint256){
        return manufacturers[_manufacturerAddr].maskAmount;
    }
}
