...
contract maskSaver is ERC721 {
     ...
     */
    function maskMaking(address _manuAddress) public {
        require(owner == msg.sender);
        
        Masks.push(Mask(now, _manuAddress));
        uint256 tokenId = Masks.length - 1; 
        _mint(owner, tokenId);
        emit Makemask(tokenId, owner, "Making masks...");
    }
    
    function maskManuApprove(uint256 tokenId) public returns (bool) {
        require (Masks[tokenId].manuAddress == msg.sender);
